...
def calculate_risk_level(suspicion_score, max_severity):
    ...
    # CRITICAL (Red): Confirmed fraud or mathematical impossibility
    # Score 80+: COA Blacklisted Contractor (+80)
    if sev == 'CRITICAL' or score >= 80:
        return ('Critical', 'Red', 'IMMEDIATE INVESTIGATION. Strong, confirmed evidence of fraud.')

    # HIGH (Yellow): Serious data anomalies
    # Score 40-79: 
    # - Invalid Timeline (+70)
    # - No Contractor (+50) 
    # - Duplicate ID (+40)
    # - Missing Cost (+40)
    # CHANGED: Lowered threshold from 60 to 40 to catch these cases
    if sev == 'HIGH' or score >= 40:
        return ('High', 'Yellow', 'PRIORITY INVESTIGATION. Serious red flags detected.')

    # LOW (Green): Minor issues or Clean
    # Score < 40: Missing Location (+30) or Clean (0)
    ...
    try:
        ...
        query = f'''
            SELECT id, project_id, project_description, contractor, contract_cost,
                ...
                is_flagged, max_severity, {score_column}, color_triage, latitude, longitude, year
            FROM projects
        ...
        for row in rows:
            ...
            # Recalculate Risk on the fly with new thresholds
            risk_level, color_name, risk_description = calculate_risk_level(score, row['max_severity'])
            ...
            projects.append({
                ...
                'status': 'Flagged' if score >= 40 else 'Normal', # Updated status threshold too
...
def get_stats():
    try:
        ...
        # Flagged Count (Using new 40 threshold)
        cursor.execute(f'''
...
def get_project_detail(project_id):
    try:
        ...
        risk_level, color_name, risk_desc = calculate_risk_level(score, project['max_severity'])
