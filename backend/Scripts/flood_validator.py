...
# ================= PATH CONFIGURATION =================
# This ensures the script finds files in ../Datas/ no matter where you run it from
...
def get_satellite_candidates(flagged_projects):
    """
    Returns ALL projects that have valid locations, regardless of score.
    LIMIT REMOVED.
    """
    candidates = []
    ...
    # Sort candidates: Higher score first
    candidates.sort(key=lambda x: (
        x.get('suspicion_score', 0) or 0), reverse=True)
...
def main():
    try:
        print("=" * 80)
        print("FLOOD CONTROL PROJECT VALIDATOR (FULL VERSION)")
        print("=" * 80)
        ...
        # Save candidates for the geocoder
        ...
        if coa_report:
            save_json(coa_report, COA_REPORT_FILE)
            print(f"✓ COA contractor report saved to: {COA_REPORT_FILE}")
        ...
        print(f"Looked for: {RAW_INPUT_FILE}")
